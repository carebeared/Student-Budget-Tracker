...
public class Tab1  extends Fragment{
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        Cursor cursor1=sqLiteDatabase.rawQuery( "SELECT  strftime('%Y-%m',dateyo) as month_of_year, sum(amount) as total FROM student_demo_p_view GROUP BY month_of_year",null);
        int i=0;

        BarChart lineChart = (BarChart) getActivity().findViewById(R.id.chart55);
        ...
        lineChart.setBorderColor(Color.parseColor("#9400D3"));
      ...
      final String[]symbol = {"","₹","₱","£","kr","$","$","Дин.","RM","Rf."};
        ...
        sqLiteDatabase.execSQL("CREATE TABLE IF NOT EXISTS student_demo_p_view(id INTEGER PRIMARY KEY AUTOINCREMENT,amount VARCHAR,dateyo VARCHAR,timeyo VARCHAR)");//create table
        Cursor cursor_yo = sqLiteDatabase.rawQuery("SELECT SUM(amount) FROM student_demo_p_view", null);
        if (cursor_yo.moveToFirst()) {
            String rupee1 = getResources().getString(R.string.Rs);
            long  amount_yo = cursor_yo.getLong(0);
            TextView t = (TextView) getActivity().findViewById(R.id.textView6);
      ...
      Cursor cursor2=sqLiteDatabase.rawQuery( "SELECT  strftime('%d-%m',dateyo) as date, sum(amount) as total1 FROM student_demo_p_view  GROUP BY date order by date DESC limit 8 ",null);
        int j=0;
        BarChart lineChart1 = (BarChart) getActivity().findViewById(R.id.chartyear1);
        ...
        lineChart1.setBorderColor(Color.parseColor("#9400D3"));
      ...
      /* // String[] xValues = {"JAN", "FEB", "MAR","APR","MAY", "JUN","JULY","AUG", "SEP,"OCT","NOV","dEC"};
        ...
        Cursor cursor11=sqLiteDatabase.rawQuery("SELECT dateyo  FROM student_total_budget_p",null);
        ArrayList<String> labels = new ArrayList<String>();
        while(cursor11.moveToNext()){
            String data2 = cursor11.getString(0);
            //Toast.makeText(getActivity(), "hjbj"+data2, Toast.LENGTH_SHORT).show();
            String str=data2.substring(5,7);
            //Toast.makeText(getActivity(), "hjbj"+str, Toast.LENGTH_LONG).show();
            String month_string[]={"JAN", "FEB", "MAR","APR","MAY", "JUN","JULY","AUG","SEP","OCT","NOV","dEC"};
           int amonth=(int)((str.charAt(0)-'0')*10+(str.charAt(1)-'0'));
            for (int j = 0; j < entries.size(); j++) {
               labels.add(month_string[amonth-1]);
                amonth++;
                if (amonth==13){
                    amonth=1;
                }
